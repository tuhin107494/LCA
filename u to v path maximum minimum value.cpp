...
///====================================StartHere=================////////////
ll cs=0;
int  par[mx5][20],mn[mx5][20],mx[mx5][20];
vector<pair<int,int> >v[mx5];
int depth[mx5],mn_val=mx9,mx_val=0;
void dfs(int u, int p,int d,int w)
{
    ...
    for(auto node: v[u])
    {
        if(node.F!=p) dfs(node.F, u,d+1,node.S);
    }
}

void calculate_parent(ll n)
{
...
int kth_parent(int u, int k)
{
    for(int b = 19; b>=0; b--)
    {
        if((k >> b) & 1)
        {
             mn_val = min(mn_val,mn[u][b]);
             mx_val = max(mx_val,mx[u][b]);
             u = par[u][b];
...
int LCA(int u, int v1)
{
    ...
    else
    {
        ...
        for(int k=19; k>=0; k--)
        {
            ...
            {
                 mn_val = min(mn_val,mn[u][k]);
                 mx_val = max(mx_val,mx[u][k]);
                 mn_val = min(mn_val,mn[v1][k]);
                 mx_val = max(mx_val,mx[v1][k]);
          ...
          mn_val = min(mn_val,mn[u][0]);
          mx_val = max(mx_val,mx[u][0]);
          mn_val = min(mn_val,mn[v1][0]);
          mx_val = max(mx_val,mx[v1][0]);
        return par[v1][0];
    }
}
